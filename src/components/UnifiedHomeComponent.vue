...
const teamDominantWins = computed(() => {
  ...
  console.log('[teamDominantWins] Result sample:', [...result.entries()].slice(0, 3))
  return result
})
...
async function downloadTeamDetailImage() {
  ...
  try {
    ...
    // Load team logo - EXACTLY like Power Rankings
    console.log(`[Download Team] Loading team logo for ${team.name}...`)
    const teamLogoBase64 = await new Promise<string>((resolve) => {
      try {
        const img = new Image()
        img.crossOrigin = 'anonymous'
        img.onload = () => {
