...
/* ─────────────────────────────────────────────────────────────────
   ...
   Power Rankings page share a single source of truth.
───────────────────────────────────────────────────────────────── */

type FactorId = keyof TeamFactorScores
...
function applyPreset(presetId: string) {
  const preset = powerRankingPresets.find((p) => p.id === presetId)
  if (!preset) return
...
// LAST WEEK's standings (seasonRankHistory week 10), so MOVE shows real
// week-over-week movement even when the user hasn't touched the weights.
const liveRankings = computed<LiveRankingEntry[]>(() => {
  ...
  // Use the previous week (currentWeek - 1) ranks as the baseline.
  const prevWeekIdx = seasonRankHistory.findIndex((w) => w.week === currentWeek - 1)
  const prevWeekRanks =
    prevWeekIdx >= 0 ? seasonRankHistory[prevWeekIdx].ranks : seasonRankHistory[seasonRankHistory.length - 2]?.ranks ?? {}
...
export function useDemoPowerRankings() {
  return {
