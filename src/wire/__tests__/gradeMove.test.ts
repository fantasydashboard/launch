...
describe('gradeLabel', () => {
  it('grades by ECW delta thresholds', () => {
    ...
    expect(gradeLabel(0.2)).toEqual({ label: 'Strong upgrade', tone: 'up' })
    expect(gradeLabel(0.1)).toEqual({ label: 'Solid upgrade', tone: 'up' })
    expect(gradeLabel(0.03)).toEqual({ label: 'Marginal upgrade', tone: 'up' })
    expect(gradeLabel(0)).toEqual({ label: 'Lateral move', tone: 'flat' })
    expect(gradeLabel(-0.01)).toEqual({ label: 'Lateral move', tone: 'flat' })
