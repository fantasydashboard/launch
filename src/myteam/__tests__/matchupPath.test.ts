...
describe('matchupPath', () => {
  ...
  it('normal case: names the coin-flip target and flags the losses as no-spend', () => {
    // 10 cats, majority 6. safe 4 -> need 2 of 4 coin-flips.
    const path = matchupPath(make(4, 4, 2))
    expect(path).toContain('Win 2 of 4 coin-flips to take the week.')
    expect(path).toContain("don't spend")
  })

  it('singularizes a one-coin-flip target', () => {
    // 10 cats, majority 6. safe 5 -> need 1 of 1 coin-flip.
    expect(matchupPath(make(5, 1, 4))).toContain('Win 1 of 1 coin-flip ')
  })

  it('omits the no-spend clause when there are no likely losses', () => {
    // 8 cats, majority 5, safe 4 -> need 1 of 4 coin-flips, no losses to flag.
    const path = matchupPath(make(4, 4, 0))!
    expect(path).toContain('Win 1 of 4 coin-flips')
  ...
  it('behind with no coin-flips: must steal a loss', () => {
    // majority 6, safe 2, no tossups.
    expect(matchupPath(make(2, 0, 8))).toContain('steal one of the likely losses')
  })

  it('coin-flip sweep still short: names how many losses to steal', () => {
