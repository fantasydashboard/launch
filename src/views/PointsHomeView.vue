    ...
    <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-dark-card to-dark-bg border border-dark-border">
      ...
      <div class="relative p-6 md:p-8">
        ...
        <div v-if="isLoading" class="flex items-center justify-center py-12">
          <LoadingSpinner size="lg" message="Loading matchups..." />
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            <button 
              ...
              @mouseover="standingsCopyState === 'idle' && ($event.currentTarget.style.background = '#facc15') && ($event.currentTarget.style.color = '#111827')"
              @mouseout="standingsCopyState === 'idle' && ($event.currentTarget.style.background = 'transparent') && ($event.currentTarget.style.color = '#facc15')"
            >
              ...
              {{ isGeneratingDownload ? 'Generating...' : standingsCopyState === 'success' ? 'Copied! 📋' : standingsCopyState === 'error' ? 'Saved instead ↓' : 'Copy Image' }}
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div v-if="isLoadingChart" class="flex items-center justify-center py-12">
          <LoadingSpinner size="md" message="Loading chart data ({{ chartLoadProgress }})..." />
        </div>
        ...
        <div class="relative bg-dark-elevated rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-dark-border">
          <div class="sticky top-0 z-10 px-6 py-4 border-b border-dark-border bg-dark-elevated flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <svg v-if="!isGeneratingLeaderDownload" class="w-4 h-4 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg>
...
async function loadAllData() {
  ...
  try {
    ...
    // For ESPN category leagues, fetch real category breakdown
    if (leagueStore.activePlatform === 'espn' && !isPointsLeague.value) {
      console.log('[loadAllData] ESPN Category League detected - fetching real category stats')
      try {
        // Extract league ID and season from league key (format: "espn_flb_12345_2024")
        const parts = leagueKey.split('_')
        const espnLeagueId = parts[2] || leagueKey
        const season = parseInt(parts[3]) || parseInt(currentSeason.value.toString())
        
        const categoryBreakdown = await espnService.getCategoryStatsBreakdown('baseball', espnLeagueId, season)
        
        console.log('[loadAllData] Got category breakdown:', {
        ...
        console.log('[loadAllData] Category wins set for', convertedCatWins.size, 'teams')
        
      } catch (error) {
        console.error('[loadAllData] Error fetching ESPN category breakdown:', error)
        // Fall back to distribution method
        if (displayCategories.value.length > 0) {
          distributeCategoryWins()
        }
      }
    } else if (!isPointsLeague.value && displayCategories.value.length > 0) {
      // For Yahoo category leagues (or as fallback), use distribution method
      distributeCategoryWins()
    }
