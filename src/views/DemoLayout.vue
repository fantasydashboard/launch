    ...
    <header class="demo-bar" role="banner">
      <div class="demo-bar-inner">
        <div class="demo-bar-left">
          <span class="demo-bar-pill" aria-label="You are exploring a demo league">
            <span class="demo-bar-dot" aria-hidden="true"></span>
            Demo mode
          </span>
          <span class="demo-bar-name">The Pillars. 10-team Sleeper league.</span>
        </div>
        <div class="demo-bar-right">
          <router-link to="/" class="demo-bar-back">Back to landing</router-link>
    ...
    <nav class="demo-nav" aria-label="Demo pages">
      <div class="demo-nav-inner">
        <router-link
          ...
          exact-active-class="demo-nav-tab-active"
        >Home</router-link>
        <router-link
          class="demo-nav-tab"
          to="/demo/power-rankings"
          active-class="demo-nav-tab-active"
...
<style scoped>
.demo-shell {
  /* Canonical OKLCH design tokens for the entire demo experience.
     Children inherit via the CSS cascade — do not redeclare in child files
     unless intentionally overriding (see TeamSeasonModal for an example). */
  ...
  --accent-tertiary:  oklch(0.72 0.18 195);  /* teal — neutral accent */
  --accent-up:        oklch(0.74 0.18 145);  /* green — wins / positive */
  --accent-down:      oklch(0.65 0.20 25);   /* red — losses / negative */
...
.demo-nav {
  position: sticky;
  top: 49px; /* sits just under the primary demo bar */
  z-index: 90;
...
@media (hover: hover) and (pointer: fine) {
  .demo-nav-tab:hover:not(.demo-nav-tab-disabled):not(.demo-nav-tab-active) {
    color: oklch(0.78 0.008 90);
  }
}
.demo-nav-tab-active {
  color: oklch(0.97 0.005 90);
  border-bottom-color: oklch(0.70 0.27 350);
}
