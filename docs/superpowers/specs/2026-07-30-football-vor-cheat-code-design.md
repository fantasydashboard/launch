...
- **Two timeframes:** ROS (primary) + a **near-term weekly** VOR (next ~1–3 weeks, bye-aware) for the streaming section.
...
- **Consumers:** Wire (best-available + lineup-marginal upgrades + weekly + full board) and Trades (fair value + surplus/need targets).
- **Baseball untouched.**
...
- **Weekly points**: `sleeperService.getWeekProjections('football', season, week)` for the next ~1–3 weeks; per player, weekly league-scored points. A team on **bye** that week → 0 (derive byes from `sleeperService.getSchedule(season, week)` — teams absent from that week's games).
...
Per player: `vorRos = pointsRos − replacement[pos]`, `vorWeek = pointsWeek − replacementWeek[pos]` (weekly baseline computed the same way on weekly points). Emits `PlayerVor { playerKey, position, pointsRos, vorRos, pointsWeek, vorWeek, confidence, opportunity }`. VOR can be negative (below replacement = not startable). This is the ranking currency.
...
- **Wire** (`pointsWire.ts` / `PointsWireView.vue`):
  ...
  - **This week** section = `vorWeek` desc among free agents (bye-aware) — replaces the baseball "stream" section for football.
...
- **Non-regression:** baseball value/Wire/Trades untouched; existing football value (My Team) still renders (VOR is additive; My Team can keep showing points while Wire/Trades adopt VOR, or adopt VOR uniformly — decided in the plan).
...
- **Modify:** projection layer (K/DEF + weekly + confidence), `usePointsValue`/value wiring to expose VOR, `pointsWire.ts` + `PointsWireView.vue` (best-available/upgrades/weekly/board), `pointsTrades.ts`/`pointsTradeLandscape.ts` + `PointsTradesView.vue` (VOR fair value + targets).

## Build stages (for the plan)

1. **VOR engine spine** — projection (ROS+weekly+K/DEF+confidence) → replacement baseline → VOR + opportunity. Pure + tested; unconsumed.
2. **Wire onto VOR** — best-available, lineup-marginal upgrades, weekly section, full board.
3. **Trades onto VOR** — fair value + surplus/need targets.
4. **Polish** — confidence/opportunity display, board UX.
